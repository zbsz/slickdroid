...
class StaticQueryInvoker[-P, +R](val getStatement: String, pconv: SetParameter[P], param: P, rconv: GetResult[R]) extends StatementInvoker[R] {
  override protected def setParam(st: PreparedStatement): Unit = pconv(param, new PositionedParameters(st))
  override protected def extractValue(rs: PositionedResult): R = rconv(rs)
}
