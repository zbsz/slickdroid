...
class MapperSpec extends SlickDroidSpec {
  ...
  scenario("testMappedEntity") {
    ...
    users.filter(_.last inSet lastNames).list.size shouldEqual 1
    ...
    Query(users.filter(_.id === 1).exists).first shouldEqual true
    ...
    users.byID(3).first shouldEqual User(Some(3), "Carl", "Carlson")
    ...
    val r1 = q1.run.head
    withClue("Element class: "+r1.getClass) { r1.isInstanceOf[User] shouldEqual true }
  }
  ...
  scenario("testMappedJoin") {
    ...
    val r = q.run.toList
    val r2: List[(A, B)] = r
    r2 shouldEqual List((A(2, 2), B(2, Some("b"))))
  }

  scenario("testCustomShape") {
    ...
    q2.run shouldEqual Vector(Pair(3, Pair(42, "bb")), Pair(2, Pair(42, "cc")))
  }
  ...
  scenario("testHList") {
    ...
    q1.run shouldEqual Vector(3 :: "bb" :: (42 :: HNil) :: HNil, 2 :: "cc" :: (42 :: HNil) :: HNil)
    ...
    q2.run shouldEqual Vector(3 :: "bb" :: (42 :: HNil) :: HNil, 2 :: "cc" :: (42 :: HNil) :: HNil)
  }

  scenario("testSingleElement") {
    import scala.slick.collection.heterogenous._
    import scala.slick.collection.heterogenous.syntax._
    ...
    ares shouldEqual "Foo"
    as.update("Foo")

    as.map(a => a :: a :: HNil).run.head shouldEqual "Foo" :: "Foo" :: HNil
